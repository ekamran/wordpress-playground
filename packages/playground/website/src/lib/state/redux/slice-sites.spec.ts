...
describe('site slug helpers', () => {
	...
	it('does not sanitize explicit slug hints while avoiding collisions', () => {
		expect(getUniqueSiteSlug('Mój Playground 🚀', [])).toBe(
			'Mój Playground 🚀'
		);
		expect(getUniqueSiteSlug('  spaced slug  ', [])).toBe(
			'  spaced slug  '
		);
		expect(
			getUniqueSiteSlug('Mój Playground 🚀', ['Mój Playground 🚀'])
		).toBe('Mój Playground 🚀-2');
	});

	it('uses a fallback when an explicit slug hint is empty', () => {
		expect(getUniqueSiteSlug('', [])).toBe('playground');
	});
});
